...
public class update_house_informationAction extends ActionSupport {
	...
	public String update_house_information() {
		...
		String str = path + "\\" + session.get("current_id").toString()
				+ new Date().getDate() + photoFileName;

		String photo_path = "G:\\Tiantianshortrent\\WebContent\\upload";
		String s = session.get("current_id").toString() + new Date().getDate()
				+ photoFileName;
		str = photo_path + "\\" + s;
		...
		// dao.House_insert(address, 0, Double.parseDouble(dayprice),
		// Double.parseDouble(housearea), surrounding, othermessage,
		// "upload\\"+s, title_textfield, session.get("current_id").toString());

		// System.out.println(House_id);
		dao.update_House_information(address, dayprice, housearea, surrounding,
				othermessage, "upload\\" + s, House_id);
		// dao.rent_list_insert(User_id, House_id, User_state, House_state,
		// End_time)
		...
		/*
		 * System.out.println(House_id); System.out.println(address);
		 * System.out.println(dayprice); System.out.println(photoFileName);
		 */
